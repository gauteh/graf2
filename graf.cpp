...
void Graf::draw () {
    ...
    float min = 0;
    int points = 0;
    for (p = (plots.begin () +1); p != plots.end (); p++) {
        if (p->get_min () < min)
            min = p->get_min ();
    ...
    SDL_Rect r; r.h = SCREEN_HEIGHT - 30; r.w = SCREEN_WIDTH - 30; r.x = 30;
    r.y = 0;
   
    if (min > 0) min = 0; // start på 0 uansett int inc_x = r.w / points; int
    inc_y = r.h / (max - min);
